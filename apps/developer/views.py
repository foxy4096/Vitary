...
from .models import DevProfile, Token, Bot, WebHook
...
def dashboard(request):
    ...
    if request.method == "POST":
        ...
        return render(request, 'developer/dashboard.html', {'form': form, 'bot_form': bot_form})
...
def bot_detail(request, id):
    ...
    return render(request, 'developer/bot/detail.html', {'bot': bot, 'bform': bform, 'uform': uform})
...
def webhook_create(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('developer_dashboard')
    form = WebHookForm()
    return render(request, 'developer/bot/webhook_form.html', {'form': form, 'bot': bot})


@login_required
def webhook_delete(request, id, webhook_id):
    webhook = get_object_or_404(WebHook, id=webhook_id)
    if webhook.bot.owner.user != request.user:
...
def webhook_edit(request, id, webhook_id):
    ...
    return render(request, 'developer/bot/webhook_form.html', {'form': form, 'webhook': webhook})