...
api = NinjaAPI(title="Vitary API", description="Vitary API", version="1.0", urls_namespace="api", auth=auth, csrf=True)
...
def vit_list(request):
    ...
    if not request.user.profile.allow_nsfw: vits = vits.exclude(nsfw=True)
    return vits
